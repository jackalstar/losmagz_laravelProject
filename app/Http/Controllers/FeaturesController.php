...
class FeaturesController extends Controller
{
    ...
    public function updateFeaturePaid(Request $request)
    {
        ...
        if ($authMode == 'disabled' && $request->checked == 'true') {
            return json_encode(['success' => false, 'message' => 'Please enable the auth mode first']);
        }

        if ($paymentMode == 'disabled' && $request->checked == 'true') {
            return json_encode(['success' => false, 'message' => 'Please enable the payment mode first']);
        }
